...
@interface UCImpowerStudentVC () <UITableViewDelegate, UITableViewDataSource, UCRStudentSelectedHeaderViewDelegate>
...
@implementation UCImpowerStudentVC
...
{
    WeakSelf(self)
    [[ClassRequest sharedInstance] getStudentsWithClassId:[NSString stringWithFormat:@"%ld", _classId] completion:^(id object, ErrorModel *error) {
        StrongSelf(self)
        if (error) {
...
- (NSMutableArray *)arrSelectedStudent
{
    if (_arrSelectedStudent == nil) {
        _arrSelectedStudent = [NSMutableArray array];
    }
    return _arrSelectedStudent;
}

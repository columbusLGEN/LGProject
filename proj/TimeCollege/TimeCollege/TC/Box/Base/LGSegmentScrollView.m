...
static CGFloat flyViewHeight = 1;
static CGFloat flyViewXOffset = 10;
...
- (void)setFlyLocationWithIndex:(NSInteger)index{
    ...
    flyFrame.origin.x = currentButton.frame.origin.x + flyViewXOffset; //= [self flyXWithIndex:index];
    flyFrame.size.width = currentButton.bounds.size.width - flyViewXOffset * 2;//[self flyWidthWithIndex:index];
...
- (void)setupUI{
    ...
    // MARK: 高亮颜色，可根据不同项目调整
    highLightColor = UIColor.TCColor_mainColor;
    ...
    for (int i = 0; i < self.segmentItems.count; i++) {
        ...
        UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(x, 0, 0, buttonHeight)];
    ...
    /// MARK: flyView的初始frame
    CGRect flyDefaultFrame = CGRectMake(firstButton.frame.origin.x + flyViewXOffset, firstButtonFrame.size.height - 2, firstButton.bounds.size.width - flyViewXOffset * 2, flyViewHeight);

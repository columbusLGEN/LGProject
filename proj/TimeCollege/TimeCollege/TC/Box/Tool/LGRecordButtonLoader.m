    ...
    if (!(array.count == 0 || array == nil)) {
        [array enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if (idx > 0) {
                ...
                [scrollView addSubview:button];
                weakSelf.btnTotalW += button.frame.size.width + marginFive;
                weakSelf.btnTotalH = btnH + marginFive;
            }
            [button addTarget:vc action:action forControlEvents:UIControlEventTouchUpInside];
        }];
    }
...
- (UIButton *)buttonWithText:(NSString *)text frame:(CGRect)frame{
    ...
    [button setTitleColor:[UIColor YBColor_6A6A6A] forState:UIControlStateNormal];
//    [button cutBorderWithBorderWidth:1 borderColor:[UIColor TCColor_mainColor] cornerRadius:15];
...
- (UIButton *)hotButtonWithText:(NSString *)text frame:(CGRect)frame{
    ...
    button.backgroundColor = [UIColor TCColor_mainColor];

...
#define LGPreviewRect CGRectMake(0, 0, kLGScreenWidth, kLGScreenHeight - 80)
...
- (IBAction)QAProfressor:(UIButton *)sender {
    NSLog(@"问专家: ");
}
- (IBAction)ListenBack:(UIButton *)sender {
    NSLog(@"听恢复: ");
}

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"拍摄症状/病历本/现场";
    
    self.btnCamera.hidden = YES;
    self.btnBack.hidden = YES;

    UIImage *image = [UIImage imageNamed:@"sc_btn_take.png"];
    self.backCenterX.constant = -(kLGScreenWidth/2/2)-image.size.width/2/2;
...
- (void)customCamera {
    ...
    self.previewLayer.frame = LGPreviewRect;//CGRectMake(0, 0, self.view.width, self.view.height);
    ...
    if (self.saveVideoUrl) {
        ...
        [assetsLibrary writeVideoAtPathToSavedPhotosAlbum:self.saveVideoUrl completionBlock:^(NSURL *assetURL, NSError *error) {
            ...
            if (error) {
                ...
                [Utility showAllTextDialog:kLGKeyWindow Text:@"保存视频到相册发生错误"];
            } else {
...
- (void)videoHandlePhoto:(NSURL *)url {
    ...
    if (!self.takeImageView) {
        self.takeImageView = [[UIImageView alloc] initWithFrame:LGPreviewRect];
...
//    self.btnBack.hidden = YES;
    ...
    self.afreshCenterX.constant = -(kLGScreenWidth/2/2);
    self.ensureCenterX.constant = kLGScreenWidth/2/2;
